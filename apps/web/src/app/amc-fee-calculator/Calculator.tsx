  ...
  return (
    <div className="grid lg:grid-cols-5 gap-6">
      ...
      <div className="lg:col-span-3 rounded-3xl border border-violet-800/30 bg-gradient-to-br from-violet-950/60 via-saffron-950/40 to-cream-50/80 p-6 sm:p-8 backdrop-blur-sm space-y-6">
        <div>
          <label className="block text-sm font-semibold text-ink-950/90 mb-2">
            AMC MCQ attempts
          </label>
            ...
            {[1, 2, 3, 4].map((n) => (
              <button
                ...
                className={`flex-1 py-3 rounded-xl font-display font-bold transition-all ${
                  cat1Attempts === n
                    ? "bg-saffron-600 text-ink-950 shadow-glow-teal"
                    : "bg-cream-100/60 text-ink-950/80 hover:bg-cream-100"
                }`}
          ...
          <p className="text-xs text-ink-950/55 mt-2">Most pass first or second sitting.</p>
        </div>
        ...
        <div className="flex items-center justify-between gap-4 p-4 rounded-2xl bg-cream-100/60 border border-ink-950/10">
          ...
          <button
            ...
            className={`relative w-12 h-7 rounded-full transition-colors ${
              includeEpic ? "bg-saffron-600" : "bg-cream-100"
            }`}
          >
            <span
        ...
        <div className="flex items-center justify-between gap-4 p-4 rounded-2xl bg-cream-100/60 border border-ink-950/10">
          ...
          <button
            ...
            className={`relative w-12 h-7 rounded-full transition-colors ${
              includeAhpra ? "bg-saffron-600" : "bg-cream-100"
            }`}
          >
            <span
              className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-transform ${
      ...
      <div className="lg:col-span-2 rounded-3xl border border-pink-800/30 bg-gradient-to-br from-pink-950/60 via-rose-950/40 to-cream-50/80 p-6 sm:p-8 backdrop-blur-sm">
        ...
        <p className="text-sm text-ink-950/65 mt-2">
          {currency !== "AUD" && (
            <>
              ≈ {fmt(totalAud, "AUD")} ·{" "}
            </>
          ...
          {breakdown.map((item) => (
            <div key={item.label} className="flex items-start justify-between gap-3 text-sm">
              <div className="min-w-0">
                ...
                {item.detail && (
                  <p className="text-xs text-ink-950/55 mt-0.5">{item.detail}</p>
                )}
              </div>
              <p className="text-ink-950/80 font-mono shrink-0">
        ...
        <p className="text-[11px] text-ink-950/55 mt-3 leading-relaxed">
