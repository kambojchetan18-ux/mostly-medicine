...
async function generateSummary(text: string): Promise<string> {
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 256,
    system: [{ type: "text", text: "Summarise the uploaded document concisely.", cache_control: { type: "ephemeral" } }],
...
export async function POST(req: NextRequest) {
  ...
  const rl = await aiRateLimit(clientKey(req, "notes:upload", user.id), { max: 10, windowMs: 60 * 60_000 });
  if (!rl.allowed) {
