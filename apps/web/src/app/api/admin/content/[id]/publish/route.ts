...
async function postToLinkedIn(caption: string, hashtags: string[]) {
  ...
  if (!res.ok) {
    ...
    console.error("[publish/linkedin] upstream error", res.status, errText.slice(0, 300));
...
async function postToInstagram(caption: string, hashtags: string[]) {
  ...
  if (!createRes.ok) {
    ...
    console.error("[publish/instagram] media create error", createRes.status, errText.slice(0, 300));
  ...
  if (!publishRes.ok) {
    ...
    console.error("[publish/instagram] publish error", publishRes.status, errText.slice(0, 300));
