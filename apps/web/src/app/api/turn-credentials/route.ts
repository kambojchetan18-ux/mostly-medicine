...
export async function GET() {
  ...
  if (!keyId || !apiToken) {
    console.error("[turn-credentials] missing env:", {
      CLOUDFLARE_TURN_KEY_ID: !keyId,
      CLOUDFLARE_TURN_API_TOKEN: !apiToken,
      seen: Object.keys(process.env).filter((k) => k.startsWith("CLOUDFLARE_")).sort(),
    });
  ...
  try {
    ...
    return NextResponse.json({ error: "TURN credential generation failed" }, { status: 502 });
  }
}
