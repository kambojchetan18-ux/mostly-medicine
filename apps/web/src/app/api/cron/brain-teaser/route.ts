...
function renderBody(args: { teaser: Teaser; specialty: Specialty; firstName: string }): string {
  ...
  const safeQuestion = escape(teaser.challenge_question);
  const safeAnswerShort = escape(teaser.challenge_answer_short);
  const safeAnswerExplanation = escape(teaser.challenge_answer_explanation);
  return `
