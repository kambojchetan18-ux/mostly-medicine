...
export async function POST(req: NextRequest) {
  ...
  const rl = await aiRateLimit(clientKey(req, "mentor", user.id), { max: 1, windowMs: RATE_WINDOW_MS });
  if (!rl.allowed) {
    return NextResponse.json(
      { error: "rate_limited" },
      {
  ...
  try {
    ...
    console.error("[mentor message error]", err instanceof Error ? err.message : err);
