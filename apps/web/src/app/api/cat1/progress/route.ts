...
export async function GET() {
  ...
  const [topicsRes, streakRes, dueRes, totalAttemptsRes, correctAttemptsRes] = await Promise.all([
    ...
    supabase.from("sr_cards").select("*", { count: "exact", head: true }).eq("user_id", user.id).lte("due", new Date().toISOString()),
  ...
  const totalAttempted = totalAttemptsRes.count ?? 0;
