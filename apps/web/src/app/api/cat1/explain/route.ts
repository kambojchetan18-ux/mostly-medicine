...
export async function POST(req: NextRequest) {
  ...
  const { allowed } = await checkAiRateLimit(user.id, "cat1-explain", { maxRequests: 60 });
...
Keep the total response under 350 words. Use plain language suitable for an AMC candidate.`;
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 600,
    system: [{ type: "text", text: "You are an expert AMC exam tutor providing detailed MCQ explanations for IMGs preparing for the Australian Medical Council exam.", cache_control: { type: "ephemeral" } }],
