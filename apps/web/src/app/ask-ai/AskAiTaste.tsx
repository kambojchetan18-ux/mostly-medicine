    ...
    <div className="rounded-3xl border border-saffron-800/40 bg-cream-50/70 backdrop-blur p-5 sm:p-7 shadow-[0_0_60px_rgba(16,185,129,0.08)]">
      ...
      <div ref={scrollRef} className="max-h-[420px] overflow-y-auto space-y-4 pr-1">
            ...
            <div className="grid sm:grid-cols-2 gap-2">
              {STARTER_PROMPTS.map((p) => (
                <button
                  ...
                  onClick={() => ask(p)}
                  disabled={loading}
                  className="text-left rounded-xl border border-ink-950/15 bg-cream-50/60 hover:bg-cream-50 hover:border-saffron-700 px-3 py-2.5 text-xs text-ink-900/80 transition disabled:opacity-50"
                >
                  {p}
                </button>
              ))}
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="rounded-2xl px-4 py-2.5 bg-cream-100/60 text-xs text-ink-900/70 border border-ink-950/15">
              Thinking…
            </div>
          </div>
        )}
      ...
      {reachedLimit ? (
        <div className="mt-5 rounded-2xl border border-saffron-700/40 bg-saffron-900/20 p-5">
          ...
          <p className="text-xs text-ink-900/80 mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link
              ...
              className="inline-flex items-center justify-center rounded-xl border border-ink-950/15 bg-cream-50/60 hover:bg-cream-50 text-ink-900 font-semibold text-sm px-4 py-2 transition"
            >
        ...
        >
          <input
            ...
            className="flex-1 rounded-xl border border-ink-950/15 bg-cream-50/70 px-4 py-2.5 text-sm text-ink-950 placeholder:text-ink-900/60 focus:border-saffron-600 focus:outline-none focus:ring-2 focus:ring-saffron-700/30"
          />
          <button
            type="submit"
