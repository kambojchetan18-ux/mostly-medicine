    ...
    <div className="rounded-3xl border border-violet-800/30 bg-gradient-to-br from-violet-950/60 via-saffron-950/40 to-cream-50/80 p-6 sm:p-10 backdrop-blur-sm">
      ...
      <div className="mb-7">
        <div className="flex justify-between items-center mb-2 text-xs">
          <span className="text-ink-950/55 uppercase tracking-widest font-bold">
      ...
      {!done && step && (
        <div>
          ...
          {step.help && <p className="text-sm text-ink-950/65 mb-5">{step.help}</p>}
          ...
          <div className="flex items-center justify-between mt-6">
            <button
              onClick={back}
              disabled={stepIdx === 0}
              className="text-sm text-ink-950/65 hover:text-ink-950 disabled:text-ink-950/40 disabled:cursor-not-allowed font-medium transition-colors"
            >
              ← Back
            </button>
            <button
              onClick={restart}
              className="text-xs text-ink-950/45 hover:text-ink-950/65 font-medium transition-colors"
            >
              Restart
            </button>
          </div>
      ...
      {done && verdict && (
        <div>
          <div className={`rounded-2xl border bg-gradient-to-br ${COLORS[verdict.status]} p-6 sm:p-7`}>
            ...
            <p className="text-ink-950/80 leading-relaxed">{verdict.summary}</p>
          </div>
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3">
            ...
            <button
              onClick={restart}
              className="flex-1 inline-flex items-center justify-center gap-2 px-7 py-3.5 rounded-2xl font-semibold text-ink-950/80 border border-ink-950/15 hover:bg-white/5 hover:border-slate-500 transition-all"
            >
              Restart wizard
            </button>
          </div>
