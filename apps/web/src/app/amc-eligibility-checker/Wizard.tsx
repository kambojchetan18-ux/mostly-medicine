...
export default function Wizard() {
  ...
  return (
    <div className="rounded-3xl border border-violet-800/30 bg-gradient-to-br from-violet-950/60 via-saffron-950/40 to-cream-50/80 p-6 sm:p-10 backdrop-blur-sm">
      {/* Progress */}
      <div className="mb-7">
        <div className="flex justify-between items-center mb-2 text-xs">
          <span className="text-ink-900/60 uppercase tracking-widest font-bold">
      ...
      {!done && step && (
        <div>
          <h2 className="font-display font-bold text-ink-950 text-xl sm:text-2xl mb-2 leading-snug">
            {step.question}
          </h2>
          {step.help && <p className="text-sm text-ink-900/70 mb-5">{step.help}</p>}
          <div className="space-y-2.5">
            {step.options.map((opt) => (
              ...
              >
                ...
                {opt.detail && <p className="text-xs text-ink-900/70 mt-1">{opt.detail}</p>}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between mt-6">
            <button
              onClick={back}
              disabled={stepIdx === 0}
              className="text-sm text-ink-900/70 hover:text-ink-950 disabled:text-ink-900/55 disabled:cursor-not-allowed font-medium transition-colors"
            >
              ← Back
            </button>
            <button
              onClick={restart}
              className="text-xs text-ink-900/55 hover:text-ink-900/70 font-medium transition-colors"
            >
              Restart
            </button>
          </div>
      ...
      {done && verdict && (
        <div>
          <div className={`rounded-2xl border bg-gradient-to-br ${COLORS[verdict.status]} p-6 sm:p-7`}>
            ...
            <h2 className="font-display font-bold text-ink-950 text-2xl sm:text-3xl mb-3">
              {verdict.title}
            </h2>
            <p className="text-ink-900/80 leading-relaxed">{verdict.summary}</p>
          </div>
          ...
          <div className="mt-7">
            ...
            <ul className="space-y-2.5">
              {verdict.next.map((n, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-ink-900/80">
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3">
            <Link
              href="/auth/signup"
              className="flex-1 inline-flex items-center justify-center gap-2 px-7 py-3.5 rounded-2xl font-display font-bold text-ink-950 transition-all hover:-translate-y-0.5"
              style={{
            ...
            <button
              onClick={restart}
              className="flex-1 inline-flex items-center justify-center gap-2 px-7 py-3.5 rounded-2xl font-semibold text-ink-900/80 border border-ink-950/15 hover:bg-white/5 hover:border-ink-950/20 transition-all"
            >
              Restart wizard
            </button>
          </div>
