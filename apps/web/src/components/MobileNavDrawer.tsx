  ...
  return (
    <>
      <button
        ...
        className="md:hidden flex items-center justify-center w-11 h-11 rounded-xl bg-white/5 border border-white/5 text-cream-50/90 hover:text-white hover:bg-white/10 active:scale-95 transition"
      >
      ...
      <aside
        ...
        className={`md:hidden fixed top-0 left-0 z-[70] h-full w-[82vw] max-w-[320px] bg-ink-950 border-r border-cream-50/10/70 py-5 px-3 overflow-y-auto transition-transform duration-300 ease-out flex flex-col ${
      ...
      >
        <div className="flex items-center justify-between mb-6 px-2">
          ...
          <button
            ...
            className="flex items-center justify-center w-11 h-11 rounded-lg text-cream-50/65 hover:text-white hover:bg-white/10 transition"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <nav className="flex-1 space-y-0.5">
          <p className="px-2 text-[9px] font-bold text-cream-50/45 uppercase tracking-widest mb-2">Study</p>
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-3 rounded-xl text-cream-50/80 ${item.hover} hover:bg-white/5 transition text-sm font-medium min-h-[44px]`}
            >
          ...
          <div className="pt-5 pb-1.5">
            <p className="px-2 text-[9px] font-bold text-cream-50/45 uppercase tracking-widest">🇦🇺 Australian Jobs</p>
          </div>
