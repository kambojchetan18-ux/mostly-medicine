...
export default async function FreeAccessCard() {
  ...
  return (
    <section className="relative z-10 max-w-6xl mx-auto px-6 sm:px-10 pb-16">
      <div className="rounded-3xl border border-saffron-800/30 bg-gradient-to-br from-saffron-950/40 via-ink-950/40 to-saffron-950/30 backdrop-blur-sm p-8 sm:p-10">
        <div className="grid md:grid-cols-5 gap-8 items-center">
            ...
            <ul className="space-y-2 text-sm">
              {perks.map((p) => (
                <li key={p.label} className="flex items-baseline gap-2.5">
                  ...
                  <span className="text-cream-50/90 font-medium">{p.daily}</span>
                  <span className="text-cream-50/65">{p.label}</span>
                </li>
              ))}
              {ALWAYS_FREE.map((item) => (
                <li key={item.label} className="flex items-baseline gap-2.5">
                  <span className="text-saffron-400 shrink-0">✓</span>
                  <span className="text-cream-50/90 font-medium">Free forever</span>
                  {item.href ? (
                    <Link href={item.href} className="text-cream-50/80 hover:text-saffron-300 underline-offset-2 hover:underline">
                      {item.label}
                    </Link>
                  ) : (
                    <span className="text-cream-50/65">{item.label}</span>
                  )}
                </li>
              ))}
            </ul>
          ...
          {features.paidTiersEnabled ? (
            <div className="md:col-span-2 rounded-2xl border border-cream-50/10 bg-ink-950/60 p-6">
              <p className="text-[10px] font-bold text-cream-50/55 uppercase tracking-widest mb-3">
                When you're ready
              </p>
              ...
              <p className="text-[11px] text-cream-50/55 mt-3">
            ...
            <div className="md:col-span-2 rounded-2xl border border-saffron-700/50 bg-gradient-to-br from-saffron-950/50 to-ink-950/60 p-6">
