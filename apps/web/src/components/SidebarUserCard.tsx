    ...
    return (
      <div className="mt-4 pt-4 border-t border-ink-950/10/50">
        <button
          ...
          disabled={loggingOut}
          className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-ink-900/70 hover:text-red-400 hover:bg-white/5 transition text-xs font-medium disabled:opacity-50"
        >
          <span>→</span>
    ...
    <div className="mt-4 pt-4 border-t border-ink-950/10/50 space-y-1.5">
      ...
      <Link href="/dashboard/profile" className="flex items-center gap-2.5 px-2 py-2 rounded-xl hover:bg-white/5 transition group">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-ink-900/80 truncate group-hover:text-ink-950 transition">{user.name}</p>
      ...
      {user.role === "admin" && (
        <Link
          ...
          className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-ink-900/60 hover:text-amber-300 hover:bg-white/5 transition text-xs font-medium"
        >
      ...
      <button
        onClick={handleLogout}
        disabled={loggingOut}
        className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-ink-900/55 hover:text-red-400 hover:bg-white/5 transition text-xs font-medium disabled:opacity-50"
      >
        <span>→</span>
        {loggingOut ? "Signing out…" : "Sign Out"}
      </button>
